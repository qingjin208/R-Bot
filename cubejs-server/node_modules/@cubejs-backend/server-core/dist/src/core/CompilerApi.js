...
class CompilerApi {
    ...
    createCompilerInstances() {
        return (0, schema_compiler_1.prepareCompiler)(this.repository, {
    ...
    async compileSchema(compilerVersion, requestId) {
        ...
        try {
            ...
            const compilers = await (0, schema_compiler_1.compile)(this.repository, {
                allowNodeRequire: this.allowNodeRequire,
                compileContext: this.compileContext,
                allowJsDuplicatePropsInSchema: this.allowJsDuplicatePropsInSchema,
                standalone: this.standalone,
    ...
    async createQueryFactory(compilers) {
        ...
        const cubeToQueryClass = Object.fromEntries(await Promise.all(cubeEvaluator.cubeNames().map(async (cube) => {
    ...
    async getSqlGenerator(query, dataSource) {
        ...
        if (dataSource !== undefined) {
            ...
            if (dataSource !== 'default' && dbType !== _dbType) {
                ...
                sqlGenerator = await this.createQueryByDataSource(compilers, query, dataSource, _dbType);
                if (!sqlGenerator) {
    ...
    hashRequestContext(context) {
        if (!context.__hash) {
            context.__hash = crypto_1.default.createHash('md5').update(JSON.stringify(context)).digest('hex');
        }
    ...
    evaluateNestedFilter(filter, cube, context, cubeEvaluator) {
        ...
        if (filter.memberReference) {
            const evaluatedValues = cubeEvaluator.evaluateContextFunction(cube, filter.values || (() => undefined), context);
    ...
    */
    async applyRowLevelSecurity(query, evaluatedQuery, context) {
            ...
            if (cubeEvaluator.isRbacEnabledForCube(cube)) {
                ...
                for (const policy of userPolicies) {
                    ...
                    (policy?.rowLevel?.filters || []).forEach(filter => {
                        ...
                        filtersMap[cubeName][policyKey].push(this.evaluateNestedFilter(filter, cube, context, cubeEvaluator));
                    });
        ...
        const rlsFilter = this.buildFinalRlsFilter(cubeFiltersPerCubePerRole, viewFiltersPerCubePerRole, hasAllowAllForCube);
        if (rlsFilter) {
    ...
    buildFinalRlsFilter(cubeFiltersPerCubePerRole, viewFiltersPerCubePerRole, hasAllowAllForCube) {
        ...
        const cubeFiltersPerPolicy = Object.keys(cubeFiltersPerCubePerRole).reduce(policyReducer(cubeFiltersPerCubePerRole), {});
        const viewFiltersPerPolicy = Object.keys(viewFiltersPerCubePerRole).reduce(policyReducer(viewFiltersPerCubePerRole), {});
    ...
    */
    ...
    async metaConfig(requestContext, options = {}) {
        ...
        const { visibilityMaskHash, cubes: patchedCubes } = await this.patchVisibilityByAccessPolicy(compilers, requestContext, cubes);
    ...
    async metaConfigExtended(requestContext, options) {
        ...
        const { cubes: patchedCubes } = await this.patchVisibilityByAccessPolicy(compilers, requestContext, compilers.metaTransformer?.cubes);
        return {
