...
class DevServer {
    cubejsServer;
    options;
    applyTemplatePackagesPromise = null;
    dashboardAppProcess = null;
    livePreviewWatcher = new cloud_1.LivePreviewWatcher();
    ...
    initDevEnv(app, options) {
        ...
        const catchErrors = (handler) => async (req, res, next) => {
            try {
                ...
                const errorString = (e.stack || e).toString();
                ...
                try {
                    ...
                    const send500ErrorString = (send500Error.stack || send500Error).toString();
        ...
        app.get('/playground/context', catchErrors((req, res) => {
            ...
            res.json({
                ...
                anonymousId: (0, shared_1.getAnonymousId)(),
        ...
        app.post('/playground/generate-schema', catchErrors(async (req, res) => {
            ...
            await fs_extra_1.default.emptyDir(path_1.default.join(options.schemaPath, 'cubes'));
            await fs_extra_1.default.emptyDir(path_1.default.join(options.schemaPath, 'views'));
            await fs_extra_1.default.writeFile(path_1.default.join(options.schemaPath, 'views', 'example_view.yml'), `# In Cube, views are used to expose slices of your data graph and act as data marts.
...
#         includes:
#           - city`);
            await Promise.all(files.map(file => fs_extra_1.default.writeFile(path_1.default.join(options.schemaPath, 'cubes', file.fileName), file.content)));
            res.json({ files });
        }));
        ...
        app.post('/playground/schema/pre-aggregation', catchErrors(async (req, res) => {
            ...
            const file = schemaConverter.getSourceFiles().find(({ cubeName: currentCubeName }) => currentCubeName === cubeName);
            if (!file) {
