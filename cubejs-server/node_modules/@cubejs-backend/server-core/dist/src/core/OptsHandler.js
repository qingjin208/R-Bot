...
class OptsHandler {
    ...
    constructor(core, createOptions, systemOptions) {
        ...
        const options = (0, lodash_clonedeep_1.default)(this.createOptions);
     ...
     */
    assertOptions(opts) {
        (0, optionsValidate_1.default)(opts);
        if (!this.isDevMode() &&
    ...
    asserOrchestratorOptions(opts) {
        if (opts.rollupOnlyMode &&
            this.isApiWorker() &&
            (0, shared_1.getEnv)('preAggregationsBuilder')) {
    ...
    defaultDriverFactory(ctx) {
        const type = (0, shared_1.getEnv)('dbType', {
            dataSource: (0, shared_1.assertDataSource)(ctx.dataSource),
        });
        return { type };
    }
    /**
    ...
    async getDriverConcurrency(ctx) {
        const type = await this
            ...
            .dbType(ctx);
        const DriverConstructor = (0, DriverResolvers_1.lookupDriverClass)(type);
     ...
     */
    queueOptionsWrapper(context, queueOptions, queueType) {
        return async (dataSource = 'default') => {
            ...
            if (options.concurrency) {
                ...
                const workerConcurrency = (0, shared_1.getEnv)('refreshWorkerConcurrency');
                ...
                const envConcurrency = (0, shared_1.getEnv)('concurrency', { dataSource });
                if (envConcurrency) {
    ...
    initializeCoreOptions(opts) {
        ...
        const externalDbType = opts.externalDbType ||
            ...
            ((0, shared_1.getEnv)('devMode') || definedExtDBVariables.length > 0) && 'cubestore' ||
            undefined;
        let externalDriverFactory = externalDbType &&
            (() => new ((0, DriverResolvers_1.lookupDriverClass)(externalDbType))({
        ...
        const externalDialectFactory = () => (typeof externalDbType === 'string' &&
            (0, DriverResolvers_1.lookupDriverClass)(externalDbType).dialectClass &&
            (0, DriverResolvers_1.lookupDriverClass)(externalDbType).dialectClass());
        if (!this.isDevMode() && (0, shared_1.getEnv)('externalDefault') && !externalDbType) {
            (0, shared_1.displayCLIWarning)('Cube Store is not found. Please follow this documentation ' +
                'to configure Cube Store ' +
                'https://cube.dev/docs/caching/running-in-production');
        }
        if (this.isDevMode() && externalDbType !== 'cubestore') {
            (0, shared_1.displayCLIWarning)(`Using ${externalDbType} as an external database is deprecated. ` +
        ...
        const options = {
            ...
            dialectFactory: (ctx) => ((0, DriverResolvers_1.lookupDriverClass)(ctx.dbType).dialectClass &&
                (0, DriverResolvers_1.lookupDriverClass)(ctx.dbType).dialectClass()),
            externalDbType,
            externalDriverFactory,
            externalDialectFactory,
            apiSecret: process.env.CUBEJS_API_SECRET,
            telemetry: (0, shared_1.getEnv)('telemetry'),
            scheduledRefreshTimeZones: (0, shared_1.getEnv)('scheduledRefreshTimezones'),
            scheduledRefreshContexts: async () => [null],
            basePath: '/cubejs-api',
            dashboardAppPath: 'dashboard-app',
            dashboardAppPort: 3000,
            scheduledRefreshConcurrency: (0, shared_1.getEnv)('scheduledRefreshQueriesPerAppId'),
            scheduledRefreshBatchSize: (0, shared_1.getEnv)('scheduledRefreshBatchSize'),
            preAggregationsSchema: (0, shared_1.getEnv)('preAggregationsSchema') ||
                (this.isDevMode()
                    ? 'dev_pre_aggregations'
                    : 'prod_pre_aggregations'),
            schemaPath: (0, shared_1.getEnv)('schemaPath'),
            scheduledRefreshTimer: (0, shared_1.getEnv)('refreshWorkerMode'),
            sqlCache: true,
            livePreview: (0, shared_1.getEnv)('livePreview'),
            ...opts,
            jwt: {
                key: (0, shared_1.getEnv)('jwtKey'),
                algorithms: (0, shared_1.getEnv)('jwtAlgorithms'),
                issuer: (0, shared_1.getEnv)('jwtIssuer'),
                audience: (0, shared_1.getEnv)('jwtAudience'),
                subject: (0, shared_1.getEnv)('jwtSubject'),
                jwkUrl: (0, shared_1.getEnv)('jwkUrl'),
                claimsNamespace: (0, shared_1.getEnv)('jwtClaimsNamespace'),
                ...opts.jwt,
            },
            fastReload: (0, shared_1.getEnv)('fastReload'),
        };
        ...
        if (options.devServer && !options.apiSecret) {
            ...
            (0, shared_1.displayCLIWarning)(`Option apiSecret is required in dev mode. Cube has generated it as ${options.apiSecret}`);
        }
    ...
    isPreAggsBuilder() {
        return (this.isDevMode() ||
            ...
            this.isApiWorker() && (0, shared_1.getEnv)('preAggregationsBuilder'));
    }
    /**
    ...
    getOrchestratorInitializedOptions(context, orchestratorOptions) {
        ...
        const clone = (0, lodash_clonedeep_1.default)(orchestratorOptions);
        // rollup only mode (querying pre-aggs only)
        clone.rollupOnlyMode = clone.rollupOnlyMode !== undefined
            ? clone.rollupOnlyMode
            : (0, shared_1.getEnv)('rollupOnlyMode');
        // query queue options
        clone.queryCacheOptions = clone.queryCacheOptions || {};
        clone.queryCacheOptions.queueOptions = this.queueOptionsWrapper(context, clone.queryCacheOptions.queueOptions, 'query');
        // pre-aggs queue options
        clone.preAggregationsOptions = clone.preAggregationsOptions || {};
        clone.preAggregationsOptions.queueOptions = this.queueOptionsWrapper(context, clone.preAggregationsOptions.queueOptions, 'pre-aggs');
