 ...
 */
function getPreAggJobHashes(table) {
 ...
 */
function getPreAggsJobsList(context, jobedPA) {
    const jobs = [];
    jobedPA.forEach((l1) => {
        l1.forEach((l2) => {
            l2.forEach((l3) => {
 ...
 */
function getPreAggJobToken(job) {
...
class RefreshScheduler {
     ...
     */
    async getSchedulerConcurrency(core, context) {
    ...
    async runScheduledRefresh(ctx, options) {
        const context = {
            ...
            requestId: `scheduler-${ctx?.requestId || (0, uuid_1.v4)()}`,
        };
     ...
     */
    async forceReconcile(context, compilerApi) {
        const orchestratorApi = await this.serverCore.getOrchestratorApi(context);
        const compilers = await compilerApi.getCompilers();
    ...
    async preAggregationPartitions(context, queryingOptions) {
        ...
        const loadConcurrency = (0, p_limit_1.default)(queryingOptions.preAggregationLoadConcurrency || 1);
        return Promise.all(preAggregations.map(preAggregation => async () => {
            ...
            const queriesForPreAggregation = !isEphemeralPreAggregation && (await Promise.all(timezones.map(async (timezone) => this.refreshQueriesForPreAggregation(context, compilerApi, preAggregation, 
    ...
    async roundRobinRefreshPreAggregationsQueryIterator(context, compilerApi, queryingOptions, queriesCache) {
    ...
    async refreshPreAggregations(context, compilerApi, queryingOptions) {
        ...
        const preAggregationsLoadCacheByDataSource = {};
        const queriesCache = {};
        await Promise.all(ramda_1.default.range(0, concurrency)
            ...
            const queryIterator = queryIteratorState?.[queryIteratorStateKey] ||
                (await this.roundRobinRefreshPreAggregationsQueryIterator(context, compilerApi, queryingOptions, queriesCache));
     ...
     */
    async postBuildJobs(context, queryingOptions) {
        const orchestratorApi = await this.serverCore.getOrchestratorApi(context);
        const preAggregations = await this.preAggregationPartitions(context, queryingOptions);
        if (queryingOptions.dateRange) {
            preAggregations.forEach(preAggregation => {
                preAggregation.partitions = preAggregation.partitions
                    ...
                    return query_orchestrator_1.PreAggregationPartitionRangeLoader.intersectDateRanges([p.buildRangeStart, p.buildRangeEnd], queryingOptions.dateRange);
                });
        ...
        const preAggregationsLoadCacheByDataSource = {};
        const jobsPromise = Promise.all(preAggregations
        ...
        return getPreAggsJobsList(context, jobedPAs).map((job) => {
     ...
     */
    async getCachedBuildJobs(context, tokens) {
        const orchestratorApi = await this.serverCore.getOrchestratorApi(context);
        const jobsPromise = Promise.all(tokens.map(async (token) => {
