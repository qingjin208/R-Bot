...
function factoryQueueDriver(cacheAndQueueDriver, queueDriverOptions) {
    switch (cacheAndQueueDriver || 'memory') {
        case 'memory':
            return new LocalQueueDriver_1.LocalQueueDriver(queueDriverOptions);
        case 'cubestore':
            ...
            return new cubestore_driver_1.CubeStoreQueueDriver(queueDriverOptions.cubeStoreDriverFactory, queueDriverOptions);
        default:
...
class QueryQueue {
     ...
     */
    streamEvents = new events_1.EventEmitter();
    constructor(redisQueuePrefix, options) {
        ...
        this.executionTimeout = options.executionTimeout || (0, shared_1.getEnv)('dbQueryTimeout');
        ...
        this.processUid = options.processUid || (0, shared_1.getProcessUid)();
    ...
    createQueryStream(key, aliasNameToMember) {
        const stream = new QueryStream_1.QueryStream({
            key,
     ...
     */
    async executeInQueue(queryHandler, queryKey, query, priority, executeOptions) {
        const options = {
        ...
        try {
            ...
            const [added, queueId, queueSize, addedToQueueTime] = await queueConnection.addToQueue(keyScore, queryKey, orphanedTime, queryHandler, query, priority, options);
            if (added > 0) {
                waitingContext = {
                    queueId,
                    spanId: options.spanId,
    ...
    async getQueries() {
        ...
        try {
            ...
            const [stalled, orphaned, active, toProcess] = await Promise.all([stalledQueries, orphanedQueries, activeQueries, toProcessQueries].map(arr => mapWithDefinition(arr)));
            const result = {
                orphaned,
                stalled,
                active,
    ...
    async getQueryStage(stageQueryKey, priorityFilter, queryStageState) {
        ...
        const queryInQueue = Object.values(allQueryDefs).find(q => this.redisHash(q.stageQueryKey) === this.redisHash(stageQueryKey) &&
            ...
            if (query && insertedCount && activated && processingLockAcquired) {
                ...
                try {
                    ...
                    switch (handler) {
                        ...
                        default:
                            executionResult = {
                                result: await this.queryTimeout(this.queryHandlers[handler](query.query, async (cancelHandler) => {
                                    try {
