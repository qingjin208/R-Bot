 ...
 */
class BaseDriver {
    testConnectionTimeoutValue = 10000;
    logger;
    /**
     * Class constructor.
     */
   ...
   `;
    ...
    informationColumnsSchemaSorter(data) {
        return data
            .map((i) => ({
            ...i,
    ...
    async tablesSchema() {
        ...
        return sortedData.reduce(this.informationColumnsSchemaReducer, {});
    }
    ...
    async extractUnloadedFilesFromS3(clientOptions, bucketName, prefix) {
        ...
        if (list) {
            if (!list.Contents) {
                ...
                const csvFiles = await Promise.all(list.Contents.map(async (file) => {
     ...
     */
    async extractFilesFromGCS(gcsConfig, bucketName, tableName) {
        const storage = new storage_1.Storage(gcsConfig.credentials
    ...
    async extractFilesFromAzure(azureConfig, bucketName, tableName) {
