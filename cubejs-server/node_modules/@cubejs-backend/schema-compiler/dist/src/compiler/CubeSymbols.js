...
exports.CubeSymbols = exports.CURRENT_CUBE_CONSTANTS = exports.CONTEXT_SYMBOLS = void 0;
...
exports.CURRENT_CUBE_CONSTANTS = ['CUBE', 'TABLE'];
class CubeSymbols {
    symbols;
    builtCubes;
    cubeDefinitions;
    ...
    constructor(evaluateViews = false) {
        this.symbols = {};
        this.builtCubes = {};
    ...
    compile(cubes, errorReporter) {
        ...
        const sortedByDependency = ramda_1.default.pipe(ramda_1.default.sortBy((c) => !!c.isView))(cubes);
    ...
    createCube(cubeDefinition) {
        ...
        const cubeObject = Object.assign({
            ...
            get joins() {
                if (!joins) {
                    ...
                    if (!Array.isArray(parentJoins)) {
                        parentJoins = Object.entries(parentJoins).map(([name, join]) => {
                            join.name = name;
                            return join;
                        });
                    }
                    ...
                    if (!Array.isArray(localJoins)) {
                        localJoins = Object.entries(localJoins).map(([name, join]) => {
                            join.name = name;
                            return join;
                        });
                    }
            ...
            get accessPolicy() {
                if (!accessPolicy) {
                    const parentAcls = cubeDefinition.extends ? ramda_1.default.clone(super.accessPolicy) : [];
    ...
    camelCaseTypes(obj) {
        ...
        members.forEach(member => {
            if (member.type && member.type.indexOf('_') !== -1) {
                member.type = (0, inflection_1.camelize)(member.type, true);
            }
            if (member.relationship && member.relationship.indexOf('_') !== -1) {
                member.relationship = (0, inflection_1.camelize)(member.relationship, true);
            }
        });
    }
        ...
        for (const preAggregation of Object.values(preAggregations)) {
            ...
            if (Object.keys(preAggregation).length > 0) {
                ...
                if (preAggregation.allowNonStrictDateRangeMatch === undefined &&
                    ...
                    preAggregation.allowNonStrictDateRangeMatch = (0, shared_1.getEnv)('allowNonStrictDateRangeMatching');
                }
                if (preAggregation.scheduledRefresh === undefined && preAggregation.type !== 'rollupJoin' && preAggregation.type !== 'rollupLambda') {
                    preAggregation.scheduledRefresh = (0, shared_1.getEnv)('scheduledRefreshDefault');
                }
                if (preAggregation.external === undefined && preAggregation.type !== 'rollupLambda') {
                    preAggregation.external =
                        ...
                        ['rollup', 'rollupJoin'].includes(preAggregation.type) &&
                            (0, shared_1.getEnv)('externalDefault');
                }
    ...
    prepareIncludes(cube, errorReporter, splitViews) {
        ...
        for (const type of types) {
            ...
            const seen = new Set(existing.map(({ type: t, memberPath, name }) => `${t}|${memberPath}|${name}`));
            const additions = [];
    ...
    membersFromCubes(parentCube, cubes, type, errorReporter, splitViews, memberSets) {
        ...
        for (const cubeInclude of cubes) {
            const fullPath = this.evaluateReferences(null, 
            // TODO recheck `cubeInclude.joinPath` typing
            cubeInclude.joinPath, { collectJoinHints: true });
            const split = fullPath.split('.');
            ...
            if (cubeInclude.includes === '*') {
                ...
                includes = cubeInclude.includes.map((include) => {
                    ...
                    const resolved = this.getResolvedMember(type, cubeReference, includedMemberName);
                    if (!resolved)
                        return undefined;
            ...
            const excludes = (cubeInclude.excludes || [])
                ...
                .filter(Boolean);
            const finalIncludes = this.diffByMember(includes.filter(Boolean), excludes);
    ...
    generateIncludeMembers(members, type, targetCube, viewAllMembers) {
        return members.map(memberRef => {
            ...
            if (type === 'measures' && resolvedMember.drillMembers && targetCube.isView) {
                ...
                const evaluatedDrillMembers = this.evaluateReferences(sourceCubeName, resolvedMember.drillMembers, { originalSorting: true });
    ...
    evaluateContextFunction(cube, contextFn, context = {}) {
        ...
        return cubeEvaluator.resolveSymbolsCall(contextFn, (name) => {
            const resolvedSymbol = this.resolveSymbol(cube, name);
            if (resolvedSymbol) {
                return resolvedSymbol;
            }
            throw new UserError_1.UserError(`Cube references are not allowed when evaluating RLS conditions or filters. Found: ${name} in ${cube.name}`);
        }, {
            contextSymbols: {
    ...
    evaluateReferences(cube, referencesFn, options = {}) {
        ...
        const fullPath = (joinHints, path) => {
            if (joinHints?.length > 0) {
                return ramda_1.default.uniq(joinHints.concat(path));
            }
            else {
                return path;
            }
    ...
    joinHints() {
        ...
        if (Array.isArray(joinHints)) {
            return ramda_1.default.uniq(joinHints);
        }
        return joinHints;
    }
    resolveSymbolsCallDeps(cubeName, sql) {
        try {
            ...
            this.resolveSymbolsCall(sql, (name) => {
                deps.push({ name });
                const resolvedSymbol = this.resolveSymbol(cubeName, name);
                if (resolvedSymbol?._objectWithResolvedProperties) {
                    return resolvedSymbol;
                }
                return '';
    ...
    resolveSymbol(cubeName, name) {
        ...
        if (exports.CONTEXT_SYMBOLS[name]) {
            // always resolves if contextSymbols aren't passed for transpile step
            const symbol = contextSymbols?.[exports.CONTEXT_SYMBOLS[name]] || {};
            // eslint-disable-next-line no-underscore-dangle
        ...
        if (sqlResolveFn) {
            if (cube) {
                cube = this.cubeReferenceProxy(this.isCurrentCube(name) ? cubeName : name, collectJoinHints ? [] : undefined);
            }
            else if (this.symbols[cubeName]?.[name]) {
                cube = this.cubeReferenceProxy(cubeName, collectJoinHints ? [] : undefined, name);
            }
        }
    ...
    cubeReferenceProxy(cubeName, joinHints, refProperty) {
        ...
        return new Proxy({}, {
            get: (v, propertyName) => {
                ...
                if (propertyName === 'toString') {
                    if (refProperty) {
                        return () => this.withSymbolsCallContext(() => sqlResolveFn(cube[refProperty], cubeName, refProperty), { ...this.resolveSymbolsCallContext, joinHints });
                    }
                    return () => {
                        if (query) {
                        ...
                        return cubeAliasFn && this.withSymbolsCallContext(() => cubeAliasFn(cube.cubeName()), { ...this.resolveSymbolsCallContext, joinHints }) || cube.cubeName();
                    };
                }
                if (propertyName === 'sql') {
                ...
                if (refProperty &&
                    ...
                    return {
                        toString: () => this.withSymbolsCallContext(() => sqlResolveFn(cube[refProperty], cubeName, refProperty, propertyName), { ...this.resolveSymbolsCallContext }),
                    };
                }
                ...
                if (typeof propertyName === 'string') {
                    throw new UserError_1.UserError(`${cubeName}${refProperty ? `.${refProperty}` : ''}.${propertyName} cannot be resolved. There's no such member or cube.`);
                }
                return undefined;
            }
        });
    ...
    cubeDependenciesProxy(parentIndex, cubeName) {
        ...
        return new Proxy({}, {
            get: (v, propertyName) => {
                ...
                if (typeof propertyName === 'string') {
                    throw new UserError_1.UserError(`${cubeName}.${propertyName} cannot be resolved. There's no such member or cube.`);
                }
                return undefined;
            }
        });
    }
    timeDimDependenciesProxy(parentIndex) {
        const self = this;
        const { depsResolveFn } = self.resolveSymbolsCallContext || {};
        return new Proxy({}, {
            get: (v, propertyName) => {
    ...
    isCurrentCube(name) {
        return exports.CURRENT_CUBE_CONSTANTS.indexOf(name) >= 0;
    }
