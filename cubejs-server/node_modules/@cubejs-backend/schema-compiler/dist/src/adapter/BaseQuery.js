...
class BaseQuery {
    ...
    extractDimensionsAndMeasures(filters = []) {
        ...
        filters.forEach(f => {
            if (f.and) {
                ...
                throw new UserError_1.UserError(`member attribute is required for filter ${JSON.stringify(f)}`);
            }
    ...
    extractFiltersAsTree(filters = []) {
        ...
        return filters.map(f => {
            if (f.and || f.or) {
                ...
                throw new UserError_1.UserError(`You cannot use dimension and measure in same condition: ${JSON.stringify(f)}`);
            }
    ...
    initFromOptions() {
        ...
        this.canUseNativeSqlPlannerPreAggregation = (0, shared_1.getEnv)('nativeSqlPlannerPreAggregations');
    ...
    get dataSource() {
        ...
        if (dataSources.length > 1 && !this.externalPreAggregationQuery()) {
            throw new UserError_1.UserError(`To join across data sources use rollupJoin with Cube Store. If rollupJoin is defined, this error indicates it doesn't match the query. Please use Rollup Designer to verify it's definition. Found data sources: ${dataSources.join(', ')}`);
        }
    ...
    get aliasNameToMember() {
        return ramda_1.default.fromPairs(this.measures.map(m => [m.unescapedAliasName(), m.measure]).concat(this.dimensions.map(m => [m.unescapedAliasName(), m.dimension])).concat(this.timeDimensions.filter(m => !!m.granularity)
        ...
        if (this.ungrouped) {
            if (!this.options.allowUngroupedWithoutPrimaryKey && this.join) {
                ...
                if (missingPrimaryKeys.length) {
                    throw new UserError_1.UserError(`Ungrouped query requires primary keys to be present in dimensions: ${missingPrimaryKeys.map(k => `'${k}'`).join(', ')}. Pass allowUngroupedWithoutPrimaryKey option to disable this check.`);
                }
            }
            if (this.measureFilters.length) {
                throw new UserError_1.UserError('Measure filters aren\'t allowed in ungrouped query');
            }
        }
    }
    get subQueryDimensions() {
        ...
        if (!this._subQueryDimensions) {
            // eslint-disable-next-line no-underscore-dangle
            this._subQueryDimensions = this.collectFromMembers(false, this.collectSubQueryDimensionsFor.bind(this), 'collectSubQueryDimensionsFor');
        }
        // eslint-disable-next-line no-underscore-dangle
    ...
    buildParamAnnotatedSql() {
        ...
        if (preAggForQuery) {
            ...
            if (cumulativeMeasures.length === 0) {
                ...
                sql = this.regularAndTimeSeriesRollupQuery(regularMeasures, multipliedMeasures, cumulativeMeasures, preAggForQuery);
            }
        }
        else {
     ...
     */
    countAllQuery(sql) {
        return `select count(*) ${this.escapeColumnName(shared_1.QueryAlias.TOTAL_COUNT)} from (\n${sql}\n) ${this.escapeColumnName(shared_1.QueryAlias.ORIGINAL_QUERY)}`;
    }
    regularAndTimeSeriesRollupQuery(regularMeasures, multipliedMeasures, cumulativeMeasures, preAggregationForQuery) {
        ...
        const toJoin = (regularAndMultiplied.length ? [
        ...
        ] : []).concat(ramda_1.default.map(
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        ([multiplied, measure]) => this.withCubeAliasPrefix(`${this.aliasName(measure.measure.replace('.', '_'))}_cumulative`, () => this.overTimeSeriesQuery((measures, filters) => this.preAggregations.rollupPreAggregation(preAggregationForQuery, measures, false, filters), measure, true)))(cumulativeMeasures));
    ...
    externalPreAggregationQuery() {
        ...
        if (!this.options.preAggregationQuery && !this.options.disableExternalPreAggregations && this.externalQueryClass) {
            ...
            return preAggregationsDescription.length > 0 && ramda_1.default.all((p) => p.external, preAggregationsDescription);
        }
        return false;
    }
    ...
    buildSqlAndParamsRust(exportAnnotatedSql) {
        ...
        const queryParams = {
            ...
            joinHints: this.options.joinHints,
            cubestoreSupportMultistage: this.options.cubestoreSupportMultistage ?? (0, shared_1.getEnv)('cubeStoreRollingWindowJoin'),
            disableExternalPreAggregations: !!this.options.disableExternalPreAggregations,
        };
        try {
            ...
            if (e.name === 'TesseractUserError') {
                throw new UserError_1.UserError(e.message);
            }
            throw e;
        }
    }
    ...
    rollingWindowToDateJoinCondition(granularity) {
        return Object.values(this.timeDimensions.reduce((acc, td) => {
    ...
    fullKeyQueryAggregate() {
        ...
        if (this.options.preAggregationQuery) {
            const allRegular = regularMeasures.concat(cumulativeMeasures
                .map(([multiplied, measure]) => (multiplied ? null : measure))
                .filter(m => !!m));
            const allMultiplied = multipliedMeasures.concat(cumulativeMeasures
                .map(([multiplied, measure]) => (multiplied ? measure : null))
                .filter(m => !!m));
            toJoin = (allRegular.length ? [
                this.withCubeAliasPrefix('main', () => this.regularMeasuresSubQuery(allRegular))
            ] : [])
                .concat(ramda_1.default.pipe(ramda_1.default.groupBy(m => m.cube().name), ramda_1.default.toPairs, ramda_1.default.map(([keyCubeName, measures]) => this.withCubeAliasPrefix(`${keyCubeName}_key`, () => this.aggregateSubQuery(keyCubeName, measures))))(allMultiplied));
        }
        else {
            toJoin =
        ...
        if (regularMeasures.length &&
            multipliedMeasures.length &&
            !cumulativeMeasures.length) {
            const cubeNames = ramda_1.default.pipe(ramda_1.default.map(m => m.cube().name), ramda_1.default.uniq, ramda_1.default.sortBy(ramda_1.default.identity));
            const regularMeasuresCubes = cubeNames(regularMeasures);
            const multipliedMeasuresCubes = cubeNames(multipliedMeasures);
            if (ramda_1.default.equals(regularMeasuresCubes, multipliedMeasuresCubes)) {
                ...
                measuresList.sort((m1, m2) => {
                    ...
                    this.measures.forEach((m, i) => {
                        if (m.measure === m1.measure) {
                            i1 = i;
                        }
                        if (m.measure === m2.measure) {
                            i2 = i;
                        }
                    });
                    return i1 - i2;
    ...
    outerMeasuresJoinFullKeyQueryAggregate(innerMembers, outerMembers, toJoin) {
        const renderedReferenceContext = {
            renderedReference: ramda_1.default.pipe(ramda_1.default.map(m => {
            ...
            }), ramda_1.default.fromPairs)(innerMembers),
        };
    ...
    childrenMultiStageContext(memberPath, queryContext) {
        ...
        if (memberDef.addGroupByReferences) {
            ...
            queryContext = {
                ...queryContext,
                dimensions: ramda_1.default.uniq(queryContext.dimensions.concat(dims)),
            };
        }
    ...
    baseWhere(filters) {
        const filterClause = filters.map(t => t.filterToWhere()).filter(ramda_1.default.identity).map(f => `(${f})`);
    ...
    minGranularity(granularityA, granularityB) {
        ...
        const aHierarchy = ramda_1.default.reverse(this.granularityParentHierarchy(granularityA));
        const bHierarchy = ramda_1.default.reverse(this.granularityParentHierarchy(granularityB));
        let lastIndex = Math.max(aHierarchy.findIndex((g, i) => g !== bHierarchy[i]), bHierarchy.findIndex((g, i) => g !== aHierarchy[i]));
    ...
    overTimeSeriesQuery(baseQueryFn, cumulativeMeasure, fromRollup) {
        ...
        const uniqDateJoinCondition = ramda_1.default.uniqBy(djc => djc[0].dimension, dateJoinCondition);
        ...
        if (!this.timeDimensions.find(d => d.granularity)) {
            ...
            uniqDateJoinCondition, fromRollup, false));
        ...
        const uniqTimeDimensionWithRanges = ramda_1.default.uniqBy(d => d.dateRange, rangedTimeDimensions);
    ...
    dateFromStartToEndConditionSql(dateJoinCondition, fromRollup, isFromStartToEnd) {
        ...
        ([d, f]) => ({
            filterToWhere: () => {
                ...
                return f(isFromStartToEnd ?
                    this.dateTimeCast(this.paramAllocator.allocateParam(timeSeries[0][0])) :
                    `${this.timeStampInClientTz(d.dateFromParam())}`, isFromStartToEnd ?
                    this.dateTimeCast(this.paramAllocator.allocateParam(timeSeries[timeSeries.length - 1][1])) :
                    `${this.timeStampInClientTz(d.dateToParam())}`, `${fromRollup ? this.dimensionSql(d) : d.convertedToTz()}`, `${this.timeStampInClientTz(d.dateFromParam())}`, `${this.timeStampInClientTz(d.dateToParam())}`, isFromStartToEnd);
            }
        }));
    }
    /**
    ...
    dimensionOnlyMeasureToHierarchy(context, m) {
        ...
        const memberNamesForMeasure = this.collectFrom([m], this.collectMemberNamesFor.bind(this), context ? ['collectMemberNamesFor', JSON.stringify(context)] : 'collectMemberNamesFor', this.queryCache);
        const cubeNamesForMeasure = ramda_1.default.pipe(ramda_1.default.map(member => this.memberInstanceByPath(member)), 
        // collectMemberNamesFor can return both view.dim and cube.dim
        ramda_1.default.filter(member => member.definition().ownedByCube), ramda_1.default.map(member => member.cube().name), 
        // Single member expression can reference multiple dimensions from same cube
        ramda_1.default.uniq)(memberNamesForMeasure);
    ...
    collectRootMeasureToHierarchy(context) {
        ...
        return ramda_1.default.fromPairs(this.measures.concat(notAddedMeasureFilters).map(m => {
            const collectedMeasures = this.collectFrom([m], this.collectMultipliedMeasures(context), context ? ['collectMultipliedMeasures', JSON.stringify(context)] : 'collectMultipliedMeasures', this.queryCache);
            if (m.expressionName && !collectedMeasures.length && !m.isMemberExpression) {
                throw new UserError_1.UserError(`Subquery measure ${m.expressionName} should reference at least one member`);
            }
    ...
    rewriteInlineCubeSql(cube, isLeftJoinCondition) {
        ...
        if (this.cubeEvaluator.cubeFromPath(cube).rewriteQueries) {
            // TODO params independent sql caching
            const parser = this.queryCache.cache(['SqlParser', sql], () => new SqlParser_1.SqlParser(sql));
    ...
    joinQuery(join, subQueryDimensions) {
        const subQueryDimensionsByCube = ramda_1.default.groupBy(d => this.cubeEvaluator.cubeNameFromPath(d), subQueryDimensions);
        const joins = join.joins.flatMap(j => {
    ...
    get filtersWithoutSubQueries() {
        if (!this.filtersWithoutSubQueriesValue) {
            this.filtersWithoutSubQueriesValue = this.allFilters.filter(f => this.collectFrom([f], this.collectSubQueryDimensionsFor.bind(this), 'collectSubQueryDimensionsFor').length === 0);
        }
    ...
    subQueryDescription(dimension) {
        ...
        if (symbol.propagateFiltersToSubQuery) {
            filters = this.filtersWithoutSubQueries.filter(f => f instanceof BaseFilter_1.BaseFilter && !(f instanceof BaseTimeDimension_1.BaseTimeDimension)).map(f => ({
                dimension: f.dimension,
                operator: f.operator,
                values: f.values
            }));
            timeDimensions = this.filtersWithoutSubQueries.filter(f => f instanceof BaseTimeDimension_1.BaseTimeDimension).map(f => ({
                dimension: f.dimension,
                dateRange: f.dateRange
            }));
            segments = this.filtersWithoutSubQueries.filter(f => f instanceof BaseSegment_1.BaseSegment).map(f => f.segment);
        }
    ...
    regularMeasuresSubQuery(measures, filters) {
        ...
        const inlineWhereConditions = [];
        const query = this.rewriteInlineWhere(() => this.joinQuery(this.join, this.collectFrom(this.dimensionsForSelect().concat(measures).concat(this.allFilters), this.collectSubQueryDimensionsFor.bind(this), 'collectSubQueryDimensionsFor')), inlineWhereConditions);
    ...
    checkShouldBuildJoinForMeasureSelect(measures, keyCubeName) {
        ...
        return measures.map(measure => {
            const memberNamesForMeasure = this.collectFrom([measure], this.collectMemberNamesFor.bind(this), 'collectMemberNamesFor');
            ...
            if (ramda_1.default.any(cubeName => keyCubeName !== cubeName, cubes)) {
                ...
                if (measuresJoin.multiplicationFactor[keyCubeName]) {
                    ...
                    throw new UserError_1.UserError(`'${measureName}' references cubes that lead to row multiplication. Please rewrite it using sub query.`);
                }
                return true;
            }
            return false;
    ...
    groupedUngroupedSelect(select, ungrouped, granularityOverride) {
        return this.evaluateSymbolSqlWithContext(select, { ungrouped, granularityOverride, overTimeSeriesAggregate: true });
    }
    ungroupedMeasureSelect(select) {
        return this.evaluateSymbolSqlWithContext(select, { ungrouped: true });
    }
    keysQuery(primaryKeyDimensions, filters) {
        const inlineWhereConditions = [];
        const query = this.rewriteInlineWhere(() => this.joinQuery(this.join, this.collectFrom(this.keyDimensions(primaryKeyDimensions), this.collectSubQueryDimensionsFor.bind(this), 'collectSubQueryDimensionsFor')), inlineWhereConditions);
        return `SELECT DISTINCT ${this.keysSelect(primaryKeyDimensions)} FROM ${query} ${this.baseWhere(filters.concat(inlineWhereConditions))}`;
    }
    keysSelect(primaryKeyDimensions) {
        return ramda_1.default.flatten(this.keyDimensions(primaryKeyDimensions)
    ...
    keyDimensions(primaryKeyDimensions) {
        ...
        return ramda_1.default.uniqBy((d) => {
    ...
    collectSubQueryDimensionsFor(fn) {
        ...
        return ramda_1.default.uniq(context.subQueryDimensions);
    }
    ...
    orderBy() {
        if (ramda_1.default.isEmpty(this.order)) {
            return '';
        }
        const orderByString = ramda_1.default.pipe(ramda_1.default.map(this.orderHashToString), ramda_1.default.reject(ramda_1.default.isNil), ramda_1.default.join(', '))(this.order);
        if (!orderByString) {
            return '';
        }
    ...
    groupByDimensionLimit() {
        ...
        if (this.rowLimit !== null) {
            if (this.rowLimit === shared_1.MAX_SOURCE_ROW_LIMIT) {
                limit = this.paramAllocator.allocateParam(shared_1.MAX_SOURCE_ROW_LIMIT);
            }
    ...
    rollupGroupByClause(dimensionNames) {
        ...
        const groupingSets = ramda_1.default.flatten(this.dimensionsForSelect().map(d => d.dimension).filter(d => !!d)).map(d => d.groupingSet);
        let result = ' GROUP BY ';
    ...
    baseSelect() {
        return ramda_1.default.flatten(this.forSelect().map(s => s.selectColumns())).filter(s => !!s).join(', ');
    }
    selectAllDimensionsAndMeasures(measures) {
        return ramda_1.default.flatten(this.dimensionsForSelect().concat(measures).map(s => s.selectColumns())).filter(s => !!s).join(', ');
    }
    /**
    ...
    evaluateSymbolSql(cubeName, name, symbol, memberExpressionType, subPropertyName) {
        ...
        try {
            if (type === 'measure') {
                ...
                const result = this.renderSqlMeasure(name, sql && this.applyMeasureFilters(this.autoPrefixWithCubeName(cubeName, sql, isMemberExpr), symbol, cubeName), symbol, cubeName, parentMeasure, orderBySql);
                if (this.safeEvaluateSymbolContext().compositeCubeMeasures ||
                    this.safeEvaluateSymbolContext().leafMeasures) {
    ...
    cubeAlias(cubeName) {
        ...
        return this.escapeColumnName(this.aliasName(`${prefix
    ...
    collectCubeNamesFor(fn) {
        ...
        this.evaluateSymbolSqlWithContext(fn, context);
        return ramda_1.default.uniq(context.cubeNames);
    }
    ...
    collectMemberNamesFor(fn) {
        ...
        this.evaluateSymbolSqlWithContext(fn, context);
        return ramda_1.default.uniq(context.memberNames);
    }
    collectAllMemberNames() {
        return ramda_1.default.flatten(this.collectFromMembers(false, this.collectMemberNamesFor.bind(this), 'collectAllMemberNames'));
    }
    collectMultipliedMeasures(context) {
        return (fn) => {
            const foundCompositeCubeMeasures = {};
            this.evaluateSymbolSqlWithContext(fn, { ...context, compositeCubeMeasures: foundCompositeCubeMeasures });
            const renderContext = {
                ...context, measuresToRender: [], foundCompositeCubeMeasures, compositeCubeMeasures: {}, rootMeasure: {}
            };
            this.evaluateSymbolSqlWithContext(fn, renderContext);
            return renderContext.measuresToRender.length ?
                ramda_1.default.uniq(renderContext.measuresToRender) :
    ...
    collectLeafMeasures(fn) {
        ...
        this.evaluateSymbolSqlWithContext(fn, context);
        return ramda_1.default.pipe(ramda_1.default.toPairs, ramda_1.default.map(([measure, isLeaf]) => isLeaf && measure), ramda_1.default.filter(ramda_1.default.identity))(context.leafMeasures);
    }
    /**
     * @template T
     * @param {() => T} fn
    ...
    renderSqlMeasure(name, evaluateSql, symbol, cubeName, parentMeasure, orderBySql) {
        ...
        if (multiplied && (symbol.type === 'countDistinct' ||
            !this.safeEvaluateSymbolContext().hasMultipliedForPreAggregation && (symbol.type === 'number' && evaluateSql === 'count(*)' ||
        ...
        if ((this.safeEvaluateSymbolContext().ungroupedAliasesForCumulative || {})[measurePath]) {
            ...
            const onGroupedColumn = this.aggregateOnGroupedColumn(symbol, evaluateSql, topLevelMerge != null ? topLevelMerge : true, measurePath);
            if (onGroupedColumn) {
    ...
    hllInit(_sql) {
        throw new UserError_1.UserError('Distributed approximate distinct count is not supported by this DB');
    }
    hllMerge(_sql) {
        throw new UserError_1.UserError('Distributed approximate distinct count is not supported by this DB');
    }
    ...
    countDistinctApprox(sql) {
        throw new UserError_1.UserError('Approximate distinct count is not supported by this DB');
    }
    ...
    renderDimensionCase(symbol, cubeName) {
        ...
        return this.caseWhenStatement(when, symbol.case.else && this.renderDimensionCaseLabel(symbol.case.else.label, cubeName));
    }
    ...
    inDbTimeZone(date) {
        return (0, shared_1.localTimestampToUtc)(this.timezone, this.timestampFormat(), date);
    }
    /**
     * @return {string}
     */
    ...
    aggSelectForDimension(cube, dimension, aggFunction) {
        const cubeNamesForTimeDimension = this.collectFrom([dimension], this.collectCubeNamesFor.bind(this), 'collectCubeNamesFor');
    ...
    preAggregationSql(cube, preAggregation) {
        return this.cacheValue(['preAggregationSql', cube, JSON.stringify(preAggregation)], () => {
            const { collectOriginalSqlPreAggregations } = this.safeEvaluateSymbolContext();
            if (preAggregation.type === 'autoRollup') {
                ...
                return this.paramAllocator.buildSqlAndParams(originalSqlPreAggregationQuery.evaluateSymbolSqlWithContext(() => originalSqlPreAggregationQuery.evaluateSql(cube, this.cubeEvaluator.cubeFromPath(cube).sql), { preAggregationQuery: true, collectOriginalSqlPreAggregations }));
            }
    ...
    parseCronSyntax(every) {
        ...
        try {
            const interval = cron_parser_1.default.parseExpression(every, opt);
            ...
            throw new UserError_1.UserError(`Invalid cron string '${every}' in refreshKey (${err})`);
        }
    }
    calcIntervalForCronString(refreshKey) {
        ...
        if (!/^(\*|\d+)? ?(\*|\d+) (\*|\d+) \* \* (\*|\d+)$/g.test(every.replace(/ +/g, ' ').replace(/^ | $/g, ''))) {
            throw new UserError_1.UserError(`Your cron string ('${every}') is correct, but we support only equal time intervals.`);
        }
        let utcOffset = 0;
        if (refreshKey.timezone || this.timezone) {
            utcOffset = moment_timezone_1.default.tz(refreshKey.timezone).utcOffset() * 60;
        }
        return {
            utcOffset,
            interval,
    ...
    everyRefreshKeySql(refreshKey, external = false) {
        ...
        if (/^(\d+) (second|minute|hour|day|week)s?$/.test(every)) {
            const utcOffset = this.timezone ? moment_timezone_1.default.tz(this.timezone).utcOffset() * 60 : 0;
    ...
    parseInterval(interval) {
        ...
        if (!intervalMatch) {
            throw new UserError_1.UserError(`Invalid interval: ${interval}`);
        }
    ...
    preAggregationInvalidateKeyQueries(cube, preAggregation, preAggregationName) {
        return this.cacheValue(['preAggregationInvalidateKeyQueries', cube, JSON.stringify(preAggregation)], () => {
            ...
            if (preAggregation.refreshKey) {
                if (preAggregation.refreshKey.sql) {
                    return [
                        preAggregationQueryForSql.paramAllocator.buildSqlAndParams(preAggregationQueryForSql.evaluateSql(cube, preAggregation.refreshKey.sql)).concat({
                            external: false,
                ...
                if (preAggregation.refreshKey.incremental) {
                    if (!preAggregation.partitionGranularity) {
                        throw new UserError_1.UserError(`Incremental refresh key can only be used for partitioned pre-aggregations but set for non-partitioned '${cube}.${preAggregationName}'`);
                    }
                    ...
                    if (preAggregationQueryForSql.timeDimensions.length &&
                        ...
                        refreshKey = this.incrementalRefreshKey(preAggregationQueryForSql, refreshKey, { window: preAggregation.refreshKey.updateWindow, refreshKeyQuery });
                    }
                }
            ...
            if (preAggregation.type === 'originalSql') {
                return this.evaluateSymbolSqlWithContext(() => this.refreshKeysByCubes([cube]), { preAggregationQuery: true });
            }
            if (!preAggregationQueryForSql.allCubeNames.find(c => {
                const fromPath = this.cubeEvaluator.cubeFromPath(c);
                return fromPath.refreshKey && fromPath.refreshKey.sql;
            })) {
                const cubeFromPath = this.cubeEvaluator.cubeFromPath(cube);
                return preAggregationQueryForSql.evaluateSymbolSqlWithContext(() => preAggregationQueryForSql.cacheKeyQueries((refreshKeyCube, [refreshKeySQL, refreshKeyQueryOptions, refreshKeyQuery]) => {
                    if (!cubeFromPath.refreshKey) {
            ...
            return preAggregationQueryForSql.evaluateSymbolSqlWithContext(() => preAggregationQueryForSql.cacheKeyQueries(), { preAggregationQuery: true });
        }, { inputProps: { collectOriginalSqlPreAggregations: [] }, cache: this.queryCache });
    }
    ...
    preAggregationStartEndQueries(cube, preAggregation) {
        ...
        return this.evaluateSymbolSqlWithContext(() => [
            this.paramAllocator.buildSqlAndParams(preAggregation.refreshRangeStart && this.evaluateSql(cube, preAggregation.refreshRangeStart.sql) ||
                this.aggSelectForDimension(timeDimension.path()[0], timeDimension, 'min')),
            this.paramAllocator.buildSqlAndParams(preAggregation.refreshRangeEnd && this.evaluateSql(cube, preAggregation.refreshRangeEnd.sql) ||
    ...
    static emptyParametrizedContextSymbols(cubeEvaluator, allocateParam) {
        return {
            filterParams: BaseQuery.filterProxyFromAllFilters(null, cubeEvaluator, allocateParam, (filter) => new BaseGroupFilter_1.BaseGroupFilter(filter)),
            filterGroup: () => '1 = 1',
            sqlUtils: {
    ...
    static renderFilterParams(filter, filterParamArgs, allocateParam, newGroupFilter, aliases) {
        ...
        // eslint-disable-next-line prefer-spread
        return filterParamArg.__column().apply(null, filterParams.map(allocateParam));
    }
    filterGroupFunction() {
        const { allFilters } = this;
        return this.filterGroupFunctionImpl(allFilters);
    ...
    filterGroupFunctionImpl(allFilters) {
        ...
        return (...filterParamArgs) => {
            const groupMembers = filterParamArgs.map(f => {
                if (!f.__member) {
                    throw new UserError_1.UserError(`FILTER_GROUP expects FILTER_PARAMS args to be passed. For example FILTER_GROUP(FILTER_PARAMS.foo.bar.filter('bar'), FILTER_PARAMS.foo.jar.filter('jar')). But found: ${f}`);
                }
                return f.__member();
            });
            ...
            const filter = BaseQuery.findAndSubTreeForFilterGroup(newGroupFilter({ operator: 'and', values: allFilters }), groupMembers, newGroupFilter, aliasesForGroupMembers);
                        ...
                        filter: (column) => ({
                            ...
                            toString() {
                                ...
                                const aliasesForGroupMembers = Object.entries(aliases)
                                    ...
                                    .map(([_key, value]) => value);
                                const filter = BaseQuery.findAndSubTreeForFilterGroup(newGroupFilter({ operator: 'and', values: allFilters }), [groupMember], newGroupFilter, aliasesForGroupMembers);
    ...
    backAliasMembers(members) {
        const query = this;
        const aliases = Object.fromEntries(members.flatMap(member => {
