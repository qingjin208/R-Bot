...
class BaseFilter extends BaseDimension_1.BaseDimension {
    static ALWAYS_TRUE = '1 = 1';
    measure;
    operator;
    values;
    camelizeOperator;
    constructor(query, filter) {
        ...
        this.camelizeOperator = inflection_1.default.camelize(this.operator).replace(/[A-Z]/, (c) => (c != null ? c : '').toLowerCase());
    }
    filterToWhere() {
    ...
    measureFilterToWhere() {
        ...
        if (measureDefinition.filters?.length || measureDefinition.drillFilters?.length) {
            return this.query.evaluateFiltersArray((measureDefinition.filters || []).concat(measureDefinition.drillFilters || []), this.query.cubeEvaluator.cubeNameFromPath(this.measure));
        }
        else {
            return null;
        }
    ...
    isWildcardOperator() {
        ...
        return (this.camelizeOperator === 'contains' ||
    ...
    filterParams() {
        ...
        if (this.isWildcardOperator()) {
            return (0, ramda_1.map)(this.escapeWildcardChars, params);
        }
        return params;
    }
    isDateOperator() {
        return (0, ramda_1.includes)(this.camelizeOperator, DATE_OPERATORS);
    }
    valuesArray() {
    ...
    likeOr(column, not, type) {
        type = type || 'contains';
        return `${(0, ramda_1.join)(not ? ' AND ' : ' OR ', this.filterParams().map(p => this.likeIgnoreCase(column, not, p, type)))}${this.orIsNullCheck(column, not)}`;
    }
    /**
    ...
    inPlaceholders() {
        return `(${(0, ramda_1.join)(', ', this.filterParams().map(p => this.allocateCastParam(p)))})`;
    }
    inWhere(column) {
    ...
    formatFromDate(date) {
        ...
        if (!date) {
            return moment_timezone_1.default.tz(date, this.query.timezone).format(`YYYY-MM-DDT00:00:00.${'0'.repeat(this.query.timestampPrecision())}`);
        }
        return moment_timezone_1.default.tz(date, this.query.timezone).format(moment_timezone_1.default.HTML5_FMT.DATETIME_LOCAL_MS);
    }
    inDbTimeZoneDateFrom(date) {
        if (date && (date === shared_1.FROM_PARTITION_RANGE || date === shared_1.TO_PARTITION_RANGE)) {
            return date;
        }
    ...
    formatToDate(date) {
        ...
        if (!date) {
            return moment_timezone_1.default.tz(date, this.query.timezone).format(`YYYY-MM-DDT23:59:59.${'9'.repeat(this.query.timestampPrecision())}`);
        }
        return moment_timezone_1.default.tz(date, this.query.timezone).format(moment_timezone_1.default.HTML5_FMT.DATETIME_LOCAL_MS);
    }
